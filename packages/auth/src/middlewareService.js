 ...
 */
export function authenticateRequest(reqOrToken, config) {
  let tokenString;
 ...
 */
export function authorizeRequest(authContext, requiredPermission, rbacData, targetOptions) {
  const evalContext = {
 ...
 */
function sendErrorResponse(res, status, title, error) {
  const payload = {
    error: title,
  ...
  return async (req, res, next) => {
    try {
      ...
      if (options?.onAuthError) {
        options.onAuthError(error, req, res);
        return;
      }
      // Return HTTP 401 Unauthorized JSON response per RBAC.md §4
      sendErrorResponse(res, 401, 'Unauthorized', error);
    }
  };
}
/**
 ...
 * @returns Reusable HTTP middleware function
 */
export function createRbacMiddleware(requiredPermission, getRbacMatrix, options) {
  return async (req, res, next) => {
    try {
      if (!req.auth) {
