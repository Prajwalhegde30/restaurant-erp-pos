 ...
 */
function sendTenantErrorResponse(res, status, title, error) {
  const payload = {
    error: title,
 ...
 */
export async function interceptPrismaQuery(extensionArgs, options) {
  ...
  if (operation === 'create' || operation === 'upsert') {
    ...
    if (dataObj && typeof dataObj === 'object' && !Array.isArray(dataObj)) {
      const existingTenantId = dataObj.tenantId;
    ...
    if (Array.isArray(dataList)) {
      for (const item of dataList) {
        if (item && typeof item === 'object') {
          const existingTenantId = item.tenantId;
          if (existingTenantId !== undefined && String(existingTenantId) !== tenantId) {
            throw new CrossTenantAccessError(tenantId, String(existingTenantId));
          }
      ...
      const existingTenantId = dataList.tenantId;
      if (existingTenantId !== undefined && String(existingTenantId) !== tenantId) {
        throw new CrossTenantAccessError(tenantId, String(existingTenantId));
      }
      dataList.tenantId = tenantId;
    }
  }
 ...
 */
export function createTenantIsolationExtension(options) {
  return {
