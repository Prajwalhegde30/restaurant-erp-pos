 ...
 */
function toTimestamp(time?: Date | string | number | null): number | null {
 ...
 */
export function isTemporalGrantActive(
 ...
 */
function normalizePermissionName(rawPermission: string): {
  base: string;
  inferredScope?: PermissionScope;
} {
  ...
  if (lastPart === 'own' || lastPart === 'branch' || lastPart === 'tenant' || lastPart === 'any') {
    return {
      ...
      inferredScope: lastPart as PermissionScope,
    };
  }
 ...
 */
function doesPermissionMatch(grantPermission: string, targetPermission: string): boolean {
...
export interface ResolvedGrant extends PermissionGrant {
  source: string; // e.g. 'role:RoleA' or 'inherited:RoleParent'
}
...
export function resolveRoleInheritance(
  roleId: string,
  roleMap: Record<string, RoleDefinition> | Map<string, RoleDefinition>,
  maxDepth = 3,
): ResolvedGrant[] {
  const visited = new Set<string>();
  const chain: { role: RoleDefinition; isInherited: boolean }[] = [];

  let currentId: string | null | undefined = roleId;
  let depth = 0;
  ...
  const merged = new Map<string, ResolvedGrant>();
...
export function evaluatePermissions(
  ...
  const applicableGrants: ResolvedGrant[] = [];
  ...
  let bestAllowGrant: ResolvedGrant | undefined;
  let bestScopeRank = -1;
