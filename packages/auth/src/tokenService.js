 ...
 * @returns Signed JWT string
 */
export function signRefreshToken(payload, config) {
  const claims = { ...payload, tokenType: 'refresh' };
 ...
 */
export function signTokenPair(accessPayload, refreshPayload, config) {
  return {
...
export function verifyAccessToken(token, config) {
  try {
    ...
    throw new TokenInvalidError(`Access token verification failed: ${err.message}`);
  }
}
/**
...
export function verifyRefreshToken(token, config) {
  try {
    ...
    throw new TokenInvalidError(`Refresh token verification failed: ${err.message}`);
  }
}
